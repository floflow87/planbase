...
export function TicketDetailPanel({ 
  ...
  // Compute ordered body section keys from fieldOrder setting
  // Maps each field key to a canonical section key (metrics01/02/03 → "metrics")
  const defaultBodyOrder = ["happyPath", "edgeCase", "metrics", "nonRegression"];
  const orderedBodySections = (() => {
    const fo = ticketViewSettings?.fieldOrder;
    if (!fo || fo.length === 0) return defaultBodyOrder;
    const seen = new Set<string>();
    const result: string[] = [];
    for (const key of fo) {
      const canonical = key.startsWith("metrics") ? "metrics" : key;
      if (defaultBodyOrder.includes(canonical) && !seen.has(canonical)) {
        seen.add(canonical);
        result.push(canonical);
      }
    }
    for (const k of defaultBodyOrder) {
    ...
    <div className="w-[400px] border-l bg-card fixed top-0 right-0 h-screen flex flex-col z-50 shadow-lg" data-testid="ticket-detail-panel">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {/* Ordered body sections — order controlled by fieldOrder in ticketViewSettings */}
          {ticket && (ticket.type === "user_story" || ticket.type === "task" || ticket.type === "bug") && orderedBodySections.map((sectionKey) => {
            if (sectionKey === "happyPath" && !isFieldHidden("happyPath")) {
              return (
                <div key="happyPath">
                  <Separator />
                  <div className="space-y-2 mt-4" data-testid="happy-path-section">
                    <Label className="text-xs text-muted-foreground font-semibold flex items-center gap-2">
            ...
            if (sectionKey === "edgeCase" && !isFieldHidden("edgeCase")) {
              return (
                <div key="edgeCase">
                  <Separator />
                  <div className="space-y-2 mt-4" data-testid="edge-case-section">
                    <Label className="text-xs text-muted-foreground font-semibold flex items-center gap-2">
            ...
            if (sectionKey === "metrics" && (!isFieldHidden("metrics01") || !isFieldHidden("metrics02") || !isFieldHidden("metrics03"))) {
              return (
                <div key="metrics">
                  <Separator />
                  <div className="space-y-3 mt-4" data-testid="metrics-section">
            ...
            if (sectionKey === "nonRegression" && !isFieldHidden("nonRegression")) {
              return (
                <div key="nonRegression">
                  <Separator />
                  <div className="space-y-2 mt-4" data-testid="non-regression-section">
                    <Label className="text-xs text-muted-foreground font-semibold flex items-center gap-2">
                      <ClipboardList className="h-3 w-3" />
                      Non-régression
                    </Label>
