...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/sprints/:id/move", requireAuth, requireRole("owner", "collaborator"), async (req, res) => {
    try {
      ...
      // Get all sprints for this backlog ordered by position
      const allSprints = await db.select().from(sprints)
        ...
        .orderBy(sprints.position, sprints.createdAt);
      ...
      const currentPosition = sprint.position ?? currentIndex;
      const otherPosition = otherSprint.position ?? newIndex;
