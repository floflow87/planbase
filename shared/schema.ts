...
import { pgTable, uuid, text, timestamp, integer, bigint, jsonb, numeric, date, check, index, uniqueIndex, primaryKey, boolean, real } from "drizzle-orm/pg-core";
...
export const RBAC_MODULES = ['crm', 'projects', 'product', 'roadmap', 'tasks', 'notes', 'documents', 'profitability', 'whiteboards', 'treasury'] as const;
...
export const roadmaps = pgTable("roadmaps", {
  ...
  projectId: uuid("project_id").references(() => projects.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  type: text("type").notNull().default("feature_based"), // 'feature_based', 'now_next_later'
...
export const insertProjectScopeItemSchema = createInsertSchema(projectScopeItems).omit({ id: true, createdAt: true, updatedAt: true, generatedEpicId: true, generatedUserStoryId: true, generatedRoadmapItemId: true }).extend({
  ...
  scopeType: z.enum(['functional', 'technical', 'design', 'gestion', 'strategy', 'discovery', 'delivery', 'devops', 'communication', 'gtm', 'autre']).default('functional').optional(),
...
export const projectFeedbacks = pgTable("project_feedbacks", {
  ...
  // V2 qualification fields
  impactUser: text("impact_user"),
